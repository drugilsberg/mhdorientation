/**
 * @file MhdOrientation.cxx
 * @brief Implementation of the base class MhdOrientation
...
namespace Mhd{
  ...
  void MhdOrientation::OrientationReader(char* InputFile)
  {
    ...
    while(in.getline(st,256))
    {
      ...
      else if(handle.find("CompressedDataSize ")!=string::npos)
      {
        pos=handle.find_first_of("=");
        handle.erase(handle.begin(),handle.begin()+pos+2);
        CompressedDataSize=atoi(handle.c_str());
      }
      ...
      else if(handle.find("AnatomicalOrientation ")!=string::npos)
      {
        pos=handle.find_first_of("=");
        handle.erase(handle.begin(),handle.begin()+pos+2);
        if(handle==AnatomicalOrientation)
          ao=1;
        else
        {
          cerr << "===================================================================================================" << endl;
          cerr << "Error: incongrous definition of " << AnatomicalOrientation << "Orienter with " << handle << " in .mhd file" << endl;
          cerr << "===================================================================================================" << endl;
          exit(1);
        }
      }
  ...
  void MhdOrientation::OrientationWriter(char* OutputFile)
  {
  ...
  void MhdOrientation::ComputeAngles()
  {
  ...
  void MhdOrientation::ComputeRotation(float* angles)
  {
  ...
  ostream &operator<<( ostream &out, const MhdOrientation &K)
  {

    if(!K.ObjectType.empty())
    {
      out << "ObjectType = ";
      out << K.ObjectType << endl;
    }

    out << "NDims = ";
    out << K.NDims << endl;

    if(!K.BinaryData.empty())
    {
      out << "BinaryData = ";
      out << K.BinaryData << endl;
    }

    out << "BinaryDataByteOrderMSB = ";
    out << K.BinaryDataByteOrderMSB << endl;

    if(!K.CompressedDataSize)
    {
      out << "CompressedDataSize = ";
      out << K.CompressedDataSize << endl;
    }

    if(!K.CompressedData.empty())
    {
      out << "CompressedData = ";
      out << K.CompressedData << endl;
    }

    out << "TransformMatrix = ";
    for(int i=0;i<9;++i)
      out << K.TransformMatrix[i] << " ";
    out << endl;
    ...
    out << "CenterOfRotation = ";
    for(int i=0;i<3;++i)
      out << K.CenterOfRotation[i] << " ";
    out << endl;


    out << "AnatomicalOrientation = ";
    out << K.AnatomicalOrientation << endl;

    out << "ElementSpacing = ";
    for(int i=0;i<3;++i)
      out << K.ElementSpacing[i] << " ";
    out << endl;
    ...
    out << "ElementType = ";
    out << K.ElementType << endl;

    out << "ElementDataFile = ";
    out << K.ElementDataFile << endl;
